...
class TFT5Block(tf.keras.layers.Layer):
    def __init__(self, config, before_encoder_block, has_relative_attention_bias=False, **kwargs):
        super().__init__(**kwargs)
        self.is_decoder = config.is_decoder
        self.before_encoder_block = before_encoder_block
        if self.before_encoder_block is not None:
            dense_inp = tf.keras.layers.Input([None, config.d_model], dtype=tf.float32)
            dense_out = self.before_encoder_block(dense_inp)
            self.before_encoder_block = tf.keras.models.Model(inputs=dense_inp, outputs=dense_out)
...
class TFT5MainLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config, embed_tokens=None, before_encoder_block=None, **kwargs):
        ...
        self.block = [
            TFT5Block(config, before_encoder_block, has_relative_attention_bias=bool(i == 0), name=f"block_._{i}")
    ...
    def call(
        ...
        if num_dims_attention_mask == 3:
            ...
            if self.is_decoder:
                ...
                extended_attention_mask = tf.expand_dims(causal_mask, axis=1) * tf.expand_dims(tf.expand_dims(attention_mask, axis=1), axis=1)
